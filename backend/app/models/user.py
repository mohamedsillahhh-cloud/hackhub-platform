...
from sqlalchemy import String, Boolean, DateTime, Enum, Text, JSON, func
...
class User(Base):
    ...
    skills: Mapped[list] = mapped_column(JSON, nullable=True, default=list)
    ...
    preferred_languages: Mapped[list] = mapped_column(JSON, nullable=True, default=list)
    preferred_frameworks: Mapped[list] = mapped_column(JSON, nullable=True, default=list)
    interest_areas: Mapped[list] = mapped_column(JSON, nullable=True, default=list)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False)

    teams = relationship("TeamMember", back_populates="user", lazy="selectin", foreign_keys=[TeamMember.user_id])
